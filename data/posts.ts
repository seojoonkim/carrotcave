...
루트의 \`plugin.json\`에는 최소한 \`$schema\`와 \`name\`을 적는다. 스킬은 \`skills/\`, MCP 서버 설정은 \`mcp.json\`이라는 고정 위치에서 찾는다. 구성요소는 따로 검증하므로 MCP 서버 하나가 실패해도 정상적인 스킬까지 함께 무너지지는 않는다. 클라이언트 전용 기능은 역도메인 네임스페이스에 격리하고, 이를 모르는 클라이언트는 무시한다. 모든 에이전트를 같게 만드는 대신 같은 부품을 발견하고 읽는 최소 계약만 정한 셈이다.
...
화면을 맨 아래까지 내렸다. 어느 답에도 ‘다만’은 없었다. 빈칸도 없었다.`,
  ...
  },
  {
    ...
    content: `나는 텔레그램 창을 열어 에이전트에게 개발을 시킨다. 어차피 코드는 보지 않게 된지 오래됐고, 사무실이나 집 밖 어디서나 가장 간편하고 반응속도 높게 개발할때 아직 이보다 효율적인 셋업은 찾지 못했다.\x20
...
그래서 오케스트레이션이라는 말만으로는 부족하다. 오케스트레이션은 주어진 실행 안에서 순서와 분기를 조정하는 일이고, 내가 보고 있는 건 그 실행을 감싸는 기억과 규칙과 코드가 경험을 거치며 다음 버전으로 넘어가는 단계다.\x20
