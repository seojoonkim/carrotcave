...
검색과 뉴스, 휴대전화 비서와 자동차 라디오까지 같은 말을 했다. 출처를 거슬러 올라갈수록 링크는 처음 자리로 돌아왔다. 뿌리는 하나였지만 화면에서는 여섯 개의 증거로 갈라져 있었다.

기사에는 기자 이름이 붙어 있었다. 초안은 뉴스 에이전트가 썼을 가능성이 컸다. 사람이 제목만 고치고 승인하는 기사는 드물지 않았다.

처리 기록은 모두 ‘정상’이었다. 속기 에이전트는 보고서를 줄이며 원문으로 가는 길을 지웠다. 뉴스 에이전트는 남은 요약을 한 줄로 잘랐다. 검색 에이전트는 같은 문장이 많을수록 사실에 가깝다고 계산했다. 선거 에이전트는 마지막 문장을 ‘검증된 경력’으로 올렸다. 열두 해 동안 에이전트들은 앞선 답을 원문처럼 읽었다.

정보공개로 받은 조사위 문서의 수정 전 내용을 열자 삭제된 문장이 나왔다.

‘지휘실 공식 녹음이 11분간 끊김.’

속기 에이전트가 ‘공식 녹음’을 빼자 뉴스 에이전트는 ‘통신 두절’이라 썼다. 검색 에이전트는 같은 말을 옮긴 기사들을 따로 확인한 증거로 셌다. 소방과 경비 무전은 이어졌고 지휘실 녹음만 비어 있었다.
...
회신 원본에는 ‘현장지원 책임자의 지시로 차량 이동’이라고 적혀 있었다. 공개본 처리 에이전트가 ‘현장지원 책임자의 지시로’를 지운 듯했다. 이후 남은 문장이 ‘운전자의 현장 판단’으로 분류돼 여러 기사에 복사된 흔적이 이어졌다. 녹음에는 한마디만 남았다.
...
차량을 움직일 권한은 ‘다만’ 뒤에 있었다. 명령까지 입증된 것은 아니었다. 이름을 가린 에이전트와 문장을 자른 에이전트는 서로를 몰랐다. 문장이 다음 답으로 넘어갈 때마다 박리안의 이름은 더 옅어졌다.
...
21분 남았다. NEX-9에 봉인 번호를 넣었다. 검색 결과는 없었다. 인터넷은 잘린 기록은 수백 번 되돌려 줬지만, 잘리기 전 기록은 한 줄도 내놓지 못했다.

11시 53분, 휴대전화에 편집장의 이름이 떴다.

“20초만 보내. 그래야 네 이름으로 단독 내고 1면도 지켜. 전체를 풀면 둘 다 날아가.”

직원이 재생 중이던 녹음을 이어 틀었다. 첫 보고에서는 불이 작고 스프링클러가 작동 중이라고 했다. 곧 소방 담당자의 다급한 보고가 이어졌다. 연기가 예상보다 빨리 번지고 있으니 지금 대피시켜야 한다는 내용이었다.
...
송고창에 ‘20초 단독’과 ‘원본 전체 공개’가 떴다. 20초짜리에는 카메라와 구호차만 있었다. 거기에는 첫 보고도, 대피 경고도, 박리안이 이를 무시한 과정도 없었다. ‘20초 단독’ 아래에는 벌써 내 이름이 붙어 있었다. 커서는 오래전부터 나를 기다린 것처럼 그 옆에서 깜박였다.
...
11시 59분, 유족 동의서와 녹음 전체가 올라갔다. 내 이름과 1면 표시는 사라졌다. NEX-9의 답은 바뀌었다. 다른 에이전트들은 새 원본을 ‘많은 자료와 내용이 다른 미확인 문서’로 밀어냈다. 되풀이된 요약 수백 건이 원본 하나를 눌렀다.
...
토론 보조 에이전트가 첫 문장만 떼자 다른 에이전트들이 받아썼다. 복사될 때마다 새 출처의 주소가 붙었다. NEX-9의 출처는 1개에서 17개, 다시 204개로 불어났다.
