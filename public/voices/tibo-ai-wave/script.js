(() => {
  ...
  const speakerNames = { 'Tibo / Matthew Berman': 'Tibo · Matthew Berman' };
  let lastFocus = null;
  const renderSegments = data => {
    if (!Array.isArray(data.segments) || data.segments.length < 1000) throw new Error('Unexpected transcript count');
