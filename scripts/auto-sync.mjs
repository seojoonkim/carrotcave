...
- 빌더를 모아 무언가를 만들게 한 해커톤·위켄드톤·빌더 이벤트를 직접 기획·주최·운영했거나 공동 제작·제작 지원으로 참여한 기록도 빌딩을 우선한다. 참가자·관람자로서 남긴 후기나 행사 주제에 대한 일반 분석만 탐험으로 분류한다.
- 개발 경험과 제작 사례가 등장하더라도, 그것을 근거로 일반화된 명제·해석·전망을 논증하는 것이 주된 독자 보상이면 탐험으로 분류한다.
- 구현 과정·시행착오·출시·운영 결과 자체가 주된 독자 보상이면 빌딩으로 분류한다. 개발 관련 단어의 유무보다 글의 중심 기능을 우선한다.
